...
# ctx_id=0 -> GPU 0 (CUDA). Requires onnxruntime-gpu properly installed.
...
print("✅ Liveness + GPU Recognition pipeline started...")
...
with mp_face.FaceMesh(
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            # approximate face ROI for texture score
        ...
        # =========================
        # Liveness update every UPDATE_INTERVAL
        # =========================
        ...
        # =========================
        # GPU recognition only if liveness is high enough
        # =========================
        recognition_active = False
        if face_found and final_score >= FINAL_SCORE_THRESHOLD:
            recognition_active = True
            # Run InsightFace on full frame - GPU accelerated
            faces = app.get(frame)
            for face in faces:
        ...
        cv2.putText(frame, f"Realness: {realness_score:5.1f}%", (30, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 215, 255), 2)
        cv2.putText(frame, f"Final: {final_score:5.1f}%", (30, 100),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        cv2.putText(frame, f"Recognition: {'ON' if recognition_active else 'OFF'}", (30, 130),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                    (0, 255, 0) if recognition_active else (0, 0, 255), 2)

        if SHOW_WINDOW:
            cv2.imshow("Face Recognition + Liveness (GPU)", frame)
            if cv2.waitKey(1) & 0xFF == 27:  # ESC
                break

cap.release()
cv2.destroyAllWindows()
print("🧠 GPU recognition stopped.")
